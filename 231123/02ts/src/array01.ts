...
// Java, C언어를 활용해야만 작성할 수 있었던 프로그래밍 코드를 이제 TS를 활용한 JS에서도 가능하게 되었다.

// TS를 활용해서 표현식문, 선언형 방식의 프로그래밍 코드도 생성할 수 있는데, 이러한 방식의 프로그래밍 형식을 "함수형 프로그래밍"이라고 한다.

// "함수형 프로그래밍"을 최적화된 결과값을 도출하려면, 순수함수의 조건에 충족해야한다.
...
// 5) 함수의 실행문 안에 콜백함수가 없어야 한다.

// 불순함수: 순수함수의 반대 ex) prompt();
...
// const는 재할당/재선언 불가능 => 그래서 이건 말이 안되는 코드.
...
// 가급적이면 얕은 복사는 하지않는것이 좋다
// 객체타입의 얕은 복사를 깊은복사 형태로 바꾸는 방법
...
// const키워드를 사용할 대, 원시타입이 아닌 객체타입의 경우에는 안에 담긴 값이 변경!
