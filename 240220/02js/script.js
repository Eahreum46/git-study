...
//객체 안에 메서드 함수를 생성할 때, class 함수 혹은 익명함수를 사용할 때에는 this객체가 가르키는 것이 바로 상위요소의 객체를 지칭하는 것이지만, 만약 화살표함수를 사용해서 객체 내 메서드 함수를 생성할 때, this객체를 사용한다며느 그 때 this객체가 가르키는 상위요소가 아닌 window 객체를 지칭하는 것입니다! 따라서 객체 내 메서드 함수를 생성할 때에는 반드시 익명함수의 형태를 띄는 것이 좋습니다!
// *true, false 값이 바뀜
...
//생성자 함수를 활용해서 프로토타입으로 틀을 만들어놓고, 차별점을 줘야하는 값을 제외하고는 나머지 요소는 이 틀안에서 공통으로 생성될 수 있도록 해주자! => 비효율적으로 객체를 생성하지않아도 될 수 있게해주자!
//생성자 함수를 통해서 프로토타입의 객체 => 프로토타입을 통해서 만들어진 객체 => 인스턴스 객체
